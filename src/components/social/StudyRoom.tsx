...
export function StudyRoom({ room, onClose }: StudyRoomProps) {
  ...
  const [notifications, setNotifications] = useState<Array<{id: string, message: string, type: 'encouragement' | 'join' | 'leave'}>>([])
  ...
  const handleFocusUpdate = (data: FocusUpdateMessage['data']) => {
    // 다른 참가자의 집중도 업데이트
  ...
  const handleEncouragement = (data: EncouragementMessageWS['data']) => {
    // 격려 메시지 표시
    console.log('격려 메시지 수신:', data)
    
    // 내가 받은 메시지인지 확인
    if (data.to_user_id === user?.id) {
      const fromParticipant = participants.find(p => p.user_id === data.from_user_id)
      const fromName = fromParticipant?.user.name || '누군가'
      
      // 알림 추가
      setNotifications(prev => [...prev, {
        id: Date.now().toString(),
        message: `${fromName}: ${data.content}`,
        type: 'encouragement'
      }])
    }
  }
  ...
  // 집중도 업데이트 전송 (Realtime)
  const sendFocusUpdate = (focusScore: number) => {
    if (!room || !user) return
  ...
  const sendEncouragement = useCallback((toUserId: string) => {
    if (!room || !user || !encouragementMessage.trim()) return
    sendEncouragementWS(toUserId, encouragementMessage, 'text')
    setEncouragementMessage('')
  }, [room, user, encouragementMessage, sendEncouragementWS])
  ...
  useEffect(() => {
    if (room && isConnected) {
      const interval = setInterval(() => {
        ...
        sendFocusUpdateWS(newFocusScore)
      }, 5000) // 5초마다 업데이트
  ...
  }, [room, isConnected, sendFocusUpdateWS])
          ...
          {notifications.slice(-3).map((notification) => (
            <div
              ...
              className={`p-3 rounded-lg shadow-lg text-sm text-white max-w-xs ${
                notification.type === 'encouragement' ? 'bg-blue-500' :
